...
from components.auth.google_login import google_login  # ✅ Nouveau composant
...
def simple_login_form():
    if st.secrets["fake"]["use_fake_auth"]:
        ...
        google_login()  # ✅ utilise le provider
    ...
    with st.sidebar:
        if "user" in st.session_state:
            if st.button("🚪 Déconnexion"):
                ...
                st.query_params.clear()  # ✅ Nettoyer l’URL après logout
                st.rerun()